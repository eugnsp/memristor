...
private:
	...
	static constexpr Point3 neighbours[n_neighbours] = {
		{1, 0, 0}, {-1u, 0, 0}, {0, 1, 0}, {0, -1u, 0}, {0, 0, 1}, {0, 0, -1u}};
...
public:
	...
	void define_shape(Point3 size, Shape_predicate is_point_available)
	{
		...
		indices_.for_each([&is_point_available](Index& index, const Point3& pt) {
	...
	void init_uniform(double filling)
	{
		...
		indices_.for_each([&available](Index index, const Point3& point) {
			if (is_empty(index))
				available.push_back(point);
		});

		occupied_.resize(n_to_fill);
		std::sample(available.begin(), available.end(), occupied_.begin(), n_to_fill,
					rnd_generator_);
