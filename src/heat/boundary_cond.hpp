...
namespace heat
{
class Dirichlet_boundary_cond
{
public:
	Dirichlet_boundary_cond(const es_fe::Mesh2& mesh, const es_geom::Linestring& ls, double value) :
		value_(value)
	{
		std::tie(vertices_, halfedges_) = es_fe::vertices_and_halfedges_in_linestrip(ls, mesh);

		std::transform(halfedges_.begin(), halfedges_.end(), std::back_inserter(edges_),
	...
	static constexpr bool constraint()
	{
		return true;
	}
