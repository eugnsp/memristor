...
public:
	...
	void init()
	{
		...
		system().variable().set_bnd_cond<0>(
			mesh(), es_geom::Linestring{{0, 0}, {width, 0}, {width, height}, {0, height}},
			params::temperature);
...
private:
	virtual void set_bnd_values() override
	{
		system().variable().for_each_ess_bnd_cond([this](const auto& bc) {
			...
			typename Base::System::template Var_edge_dofs<0> edge_dofs;
...
public:
	...
	void assemble_on_face(const es_fe::Mesh2::Face_view& face)
	{
		...
		const auto factor = es_geom::area(face) * (
			tag == Mesh_tags::GRANULE ? 2 * params::thermal_conductivity : params::thermal_conductivity) * es_geom::center(face).x();
		...
		if (es_geom::is_greater_equal(br.left(), params::heat_source_radius))
			return;
